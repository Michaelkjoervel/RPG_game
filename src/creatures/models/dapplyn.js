...
export function build_dapplyn(kit = kitDefault) {
  ...
  const legDefs = [
    [0.09, -0.113, 0.18], [-0.09, -0.113, 0.18],
    [0.09, -0.113, -0.18], [-0.09, -0.113, -0.18],
  ];
  const legs = legDefs.map(([x, y, z]) => kit.at(body, kit.leg(0.36, skin, { thighR: 0.04, shinR: 0.028, footLen: 0.075 }), x, y, z));

  const tail = kit.at(body, kit.tailChain(2, skin, { segLen: 0.04, startR: 0.028, endR: 0.014 }), 0, 0.07, -0.24);
  ...
  for (let i = 0; i < 10; i++) {
    ...
    // half-width at height y is sqrt(r^2 - y^2) (r = 0.13).
    const x = side * Math.sqrt(Math.max(0.0169 - y * y, 0.0016));
    spotDefs.push([x, y, -0.12 + rng() * 0.24, 0.014 + rng() * 0.01, i + 1, side]);
  }
