...
// (Design Bible §4)
// =============================================================================
// Visual pass v2 (soft stylized). A small, friendly cat-sized dragon on
// four light legs: a dusk-teal body with a cream plated belly, a curving
...
export function build_glowvern(kit = kitDefault) {
  ...
  // --- Body: a cat-sized dragon on four legs, plated cream belly, spines. ------
  const torso = S.spindle({
    len: 0.34, r: 0.09, sx: 0.9, sy: 1.04, p: 0.93, radial: 16, rings: 12, belly: 0.1,
    profile: (t) => 0.8 + 0.12 * S.bump(t, 0.3, 0.3) + 0.2 * S.bump(t, 0.72, 0.3),
    arch: (t) => 0.015 * S.bump(t, 0.7, 0.25),
  });
  ...
  for (let i = 0; i < 6; i++) {
    ...
    S.aim(g, [0, 1, -0.6 - u * 0.3]);
    const at = i < 2
      ? S.surface(neck, [0, 0.4, -1], { from: [0, 0.15 - i * 0.07, 0.23 - i * 0.05], inset: 0.006 })
      : S.surface(torso, [0, 1, 0], { from: [0, 0, 0.1 - (i - 2) * 0.07], inset: 0.006 });
  ...
  const headGeo = S.spindle({
    len: 0.18, r: 0.074, sx: 1.05, sy: 0.95, pTail: 1, pNose: 1.2, radial: 16, rings: 12,
  ...
  kit.at(body, head, 0, 0.21, 0.29, { rx: 0.1 });
  ...
  // --- Lantern-paper membrane wings, raised up and back. ------------------------------
  const wings = [1, -1].map((side) => {
    const w = S.batWing(0.4, membrane, skin, {
  ...
  const cap = S.paint(S.ball(0.03, { sy: 0.6, radial: 10, rings: 6 }), BRONZE);
  cap.translate(0, -0.032, 0);
  lantern.add(new THREE.Mesh(S.merge([hanger, cap]), skin));
  const bell = new THREE.Mesh(bellGeo(0.14, 0.07), glass);
  bell.name = 'glassBell';
  bell.position.y = -0.03;
  lantern.add(bell);
  const flame = new THREE.Mesh(S.ball(0.026, { sy: 1.4, radial: 8, rings: 6 }), flameMat);
  flame.name = 'bellFlame';
  flame.position.y = -0.11;
  lantern.add(flame);
  const bellGlow = S.glow(0xffd070, 0.42, 0.55);
  bellGlow.position.y = -0.105;
  lantern.add(bellGlow);
  const heart = kit.heartspark(0.022, pal.eye, { seed: 191 });
  kit.at(lantern, heart, 0, -0.105, 0);
