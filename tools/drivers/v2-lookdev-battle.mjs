// v2 LOOK-DEV battle driver (tour-c pattern): seeded save -> Continue ->
// wild battle in Dawnmeadow's arena. Shots of the opening framing, the move
...
export async function run(page, h) {
  ...
  await page.evaluate(`(() => {
    ...
    window.LF.game.overworld.startWildBattle('${WILD}', 12);
  })()`);
  // The intro (title card, camera sweep, send-outs) runs on the clamped game
  // clock, which crawls under software GL — wait until the action ring is live.
  ...
  await h.press('Enter');
  for (let i = 0; i < 3; i++) { await h.sleep(900); await h.shot('beat'); }
  await h.waitFor(`!!document.querySelector('.bui-ring-wrap:not(.hidden) .bui-card')`, 90000, 700);
  await h.sleep(800);
